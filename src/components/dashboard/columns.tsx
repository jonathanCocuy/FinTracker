...
export function createColumns(t: (key: string) => string): ColumnDef<Transaction>[] {
  return [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <span className="font-medium truncate max-w-[150px]">{row.getValue("description")}</span>
          </div>
        )
      },
    },
    {
      accessorKey: "date",
      header: () => <div className="text-left">{t("table.date")}</div>,
      cell: ({ row }) => {
    ...
    {
      accessorKey: "amount",
      header: () => <div className="text-left">{t("table.amount")}</div>,
      cell: ({ row }) => {
        ...
        return (
          <div className={`text-left font-bold ${type === 'income' ? 'text-emerald-500' : 'text-rose-500'}`}>
    ...
    },
    {
      ...
      header: () => <div className="text-left">{t("table.category")}</div>,
      cell: ({ row }) => {
